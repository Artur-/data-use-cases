...
public interface ProductRepository extends JpaRepository<Product, Integer> {
    @Query(value = "SELECT NEW com.vaadin.artur.datausecases.gridwithaggregateddata.data.endpoint.ProductWithSales(product.name, product.price, product.category, sum(saleRow.sum) as salesLastMonth) FROM Product product LEFT JOIN SaleRow saleRow ON saleRow.product = product.id GROUP BY product.id")
