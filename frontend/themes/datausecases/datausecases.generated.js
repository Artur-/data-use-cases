...
window.Vaadin['_vaadintheme_datausecases_globalCss'] = window.Vaadin['_vaadintheme_datausecases_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin['_vaadintheme_datausecases_globalCss'].length === 0) || (!window.Vaadin['_vaadintheme_datausecases_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin['_vaadintheme_datausecases_globalCss'].push(target);
  }
  if (!document['_vaadintheme_datausecases_componentCss']) {
    
    document['_vaadintheme_datausecases_componentCss'] = true;
  }
